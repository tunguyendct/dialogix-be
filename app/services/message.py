...
async def list_messages_by_chat_id(chat_id) -> List[Message]:
  return await message_collection.find({chat_id: chat_id}).to_list(100) # type: ignore

async def get_message_by_id(id) -> Message | None:
  return await message_collection.find_one({id: id}) # type: ignore